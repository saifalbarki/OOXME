(() => {
  ...
  const statusDetailsCopy = { 'Google OAuth credentials required': 'بيانات اعتماد جوجل مطلوبة', 'Database URL required': 'رابط قاعدة البيانات مطلوب', 'Server-side Meta access required': 'يتطلب وصول ميتا من الخادم', 'Status check timed out': 'انتهت مهلة فحص الحالة', 'Status response missing': 'استجابة الحالة مفقودة' };
  let language = 'en';
  const status = value => {
    ...
    if (statusDetailsCopy[text]) return statusDetailsCopy[text];
