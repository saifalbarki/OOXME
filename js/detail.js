...
function enhanceServiceDetail(){
 ...
 placeholder.innerHTML=`<p class="eyebrow">${isArabic?'كيف نساعد':'How we help'}</p><h2>${localized[1]}</h2><ul>${items.map(item=>`<li>${item}</li>`).join('')}</ul><p class="service-price"><span>${isArabic?'السعر':'Price'}</span><strong>${isArabic?'حسب نطاق المشروع':'Tailored to your scope'}</strong></p><a class="primary-button" href="/consultation">${isArabic?'ابدأ محادثة':'Start a conversation'}</a>`;
}
...
if(detailKey==='project-albasri-commercial-group'){
 ...
 const albasriPanelCopy={en:{mall:{title:'Mall Albasri',description:'A vibrant retail content direction that makes everyday offers, products, and store moments clear, colourful, and memorable.'},hfadhi:{title:'Hfadhi',description:'A considered identity and visual direction designed to give Hfadhi a clear, confident presence across its brand materials.'},lccd:{title:'LCCD',description:"A distinctive visual system built to bring LCCD's work together with clarity, character, and a cohesive brand presence."},next:'Next',more:'See More',label:'Brands We Created',gallery:'Swipe through project images',image:'Project image'},ar:{mall:{title:'مول البصري',description:'توجه محتوى تجزئة نابض بالحياة يجعل العروض اليومية والمنتجات ولحظات المتجر واضحة وملونة ولا تُنسى.'},hfadhi:{title:'الحفاظي',description:'هوية وتوجه بصري مدروسان يمنحان الحفاظي حضورًا واضحًا وواثقًا عبر مواده وعناصر علامته.'},lccd:{title:'إل سي سي دي',description:'نظام بصري مميز يجمع أعمال إل سي سي دي بوضوح وشخصية وحضور متماسك للعلامة.'},next:'التالي',more:'المزيد',label:'العلامات التي أنشأناها',gallery:'اسحب لاستعراض صور المشروع',image:'صورة المشروع'}};
 const localizeAlbasriPanels=()=>{const language=detailRoot.lang==='ar'?'ar':'en';const copy=albasriPanelCopy[language];document.querySelectorAll('[data-project-panel]').forEach(panel=>{const panelCopy=copy[panel.dataset.projectPanel];if(!panelCopy)return;const label=panel.querySelector('.eyebrow');const title=panel.querySelector('h2');const description=panel.querySelector('.albasri-intro>p:last-child');if(label)label.textContent=copy.label;if(title)title.textContent=panelCopy.title;if(description)description.textContent=panelCopy.description;panel.querySelectorAll('img').forEach((image,index)=>image.alt=`${panelCopy.title} ${copy.image} ${index+1}`)});document.querySelectorAll('.project-next-control span').forEach(node=>node.textContent=copy.next);document.querySelectorAll('.project-reopen-control span').forEach(node=>node.textContent=copy.more);document.querySelectorAll('[data-albasri-stack]').forEach(stack=>stack.setAttribute('aria-label',copy.gallery))};
 const setAlbasriLanguage=()=>{const language=detailRoot.lang==='ar'?'ar':'en';const copy=albasriCopy[language];document.title=`${copy.title} — OOXME`;document.querySelectorAll('[data-albasri-copy]').forEach(node=>{const value=copy[node.dataset.albasriCopy];if(value)node.textContent=value});localizeAlbasriPanels()};
 setAlbasriLanguage();detailLanguageButton?.addEventListener('click',()=>requestAnimationFrame(setAlbasriLanguage));new MutationObserver(setAlbasriLanguage).observe(detailRoot,{attributes:true,attributeFilter:['lang','dir']});
 ...
 const addSeeMore=(key,container,reopen)=>{
  ...
  if(!control){control=document.createElement('button');control.type='button';control.className='project-reopen-control';control.dataset.projectReopen=key;control.innerHTML='<span>See More</span><i aria-hidden="true"></i>';container.append(control);control.addEventListener('click',reopen);localizeAlbasriPanels()}
  control.classList.add('is-visible');
 };
 const openAlbasriProject=(projectKey,behavior='smooth')=>{
  const nextPanel=document.querySelector(`[data-project-panel="${projectKey}"]`);
  if(!nextPanel)return;
  const currentPanel=document.querySelector(`[data-project-panel="${projectKey}"]`)?.previousElementSibling?.matches?.('[data-project-panel]')
   ? document.querySelector(`[data-project-panel="${projectKey}"]`).previousElementSibling
   : null;
  if(currentPanel){
  ...
  requestAnimationFrame(()=>nextPanel.scrollIntoView({behavior:window.matchMedia('(prefers-reduced-motion: reduce)').matches?'auto':behavior,block:'start'}));
 };
 document.querySelectorAll('[data-open-project]').forEach(button=>button.addEventListener('click',()=>openAlbasriProject(button.dataset.openProject)));
 const openAlbasriProjectFromHash=()=>{
  const projectKey={
   'mall-project':'mall',
   'hfadhi-project':'hfadhi',
   'lccd-project':'lccd'
  }[window.location.hash.slice(1)];
  if(projectKey)openAlbasriProject(projectKey,'auto');
 };
 window.addEventListener('hashchange',openAlbasriProjectFromHash);
 window.setTimeout(openAlbasriProjectFromHash,0);
}
...
if(detailKey==='project-sawa-university'){
 ...
 const updateSawaDetail=()=>{const arabic=detailRoot.lang==='ar';document.title=arabic?'جامعة ساوا — اوكسوم':'Sawa University — OOXME';if(sawaTitle)sawaTitle.textContent=arabic?'جامعة ساوا':'Sawa University';if(sawaDescription)sawaDescription.textContent=arabic?'هوية بصرية راقية لجامعة ساوا، تجمع بين التميّز الأكاديمي ولغة بصرية معاصرة وواضحة.':'A refined visual identity created for Sawa University, blending academic distinction with a clear, contemporary visual language.';if(sawaNext)sawaNext.textContent=arabic?'التالي':'Next';const projectSections=[{id:'next-project',en:{title:'VIIR',description:'A distinctive identity system that gives VIIR a clear, confident, and cohesive visual presence.',gallery:'VIIR project gallery',image:'VIIR project image',choose:'Show image'},ar:{title:'فير',description:'نظام هوية مميز يمنح VIIR حضورًا بصريًا واضحًا وواثقًا ومتماسكًا.',gallery:'معرض مشروع فير',image:'صورة مشروع فير',choose:'عرض الصورة'}},{id:'hi-project',en:{title:'Dr. Hamid Ibrahim',description:'A professional identity system designed to communicate clarity, confidence, and trust.',gallery:'Dr. Hamid Ibrahim project gallery',image:'Dr. Hamid Ibrahim project image',choose:'Show image'},ar:{title:'د. حميد إبراهيم',description:'نظام هوية احترافي صُمم ليعبّر عن الوضوح والثقة والمصداقية.',gallery:'معرض مشروع د. حميد إبراهيم',image:'صورة مشروع د. حميد إبراهيم',choose:'عرض الصورة'}}];projectSections.forEach(({id,en,ar})=>{const copy=arabic?ar:en;const section=document.getElementById(id);if(!section)return;const heading=section.querySelector('h2');const description=section.querySelector('.sawa-intro p');const gallery=section.querySelector('.sawa-carousel');if(heading)heading.textContent=copy.title;if(description)description.textContent=copy.description;if(gallery){gallery.setAttribute('aria-label',copy.gallery);gallery.querySelectorAll('img').forEach((image,index)=>image.alt=`${copy.image} ${index+1}`)}section.querySelectorAll('.sawa-carousel-dots button').forEach((button,index)=>button.setAttribute('aria-label',`${copy.choose} ${index+1}`));section.querySelector('.sawa-next')?.setAttribute('aria-label',arabic?'عرض المشروع التالي':'View the next project')})};
 updateSawaDetail();
