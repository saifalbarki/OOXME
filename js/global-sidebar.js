(()=>{
  ...
  if(!menu){
    ...
    menu.innerHTML='<div class="sidebar-intro"><p>Hello</p></div><div class="sidebar-links"><a href="index.html">Home</a><a href="portfolio.html">Portfolio</a><a href="services.html">Services</a><a href="consultation.html">Consultation</a><span class="sidebar-phrase">Make your next move meaningful.</span></div>';
    document.body.append(menu);
  }

  const legacyPhrase=menu.querySelector('.sidebar-intro span');
  const sidebarLinks=menu.querySelector('.sidebar-links');
  if(legacyPhrase&&sidebarLinks){
    legacyPhrase.className='sidebar-phrase';
    sidebarLinks.append(legacyPhrase);
  }
  ...
  trigger.addEventListener('click',()=>{syncMenuLanguage();const open=!menu.classList.contains('is-open');menu.classList.toggle('is-open',open);trigger.setAttribute('aria-expanded',String(open));trigger.setAttribute('aria-label',open?'Close menu':'Open menu');document.body.classList.toggle('menu-open',open)});
