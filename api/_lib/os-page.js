...
const panelThreeDiscountScript = `<script>(()=>{const mode=document.querySelector('[data-os-panel-three-mode]'),type=document.querySelector('[data-os-panel-three-type]'),list=document.querySelector('[data-os-panel-three-list]');if(!mode||!type||!list)return;const t=key=>window.OOXMEOS?.copy?.[window.OOXMEOS.language]?.[key]||key;const endpoint='/api/os/index?route=promotions';let action='',selectedId='',deleteConfirm=false,codes=[];const load=async()=>{const response=await fetch(endpoint,{credentials:'same-origin'});if(!response.ok)throw new Error('promotion_load_failed');codes=await response.json();if(!Array.isArray(codes))codes=[];};const remove=async()=>{const response=await fetch(endpoint,{method:'POST',credentials:'same-origin',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'delete',id:selectedId})});const result=await response.json().catch(()=>({}));if(!response.ok)throw new Error(result.error||'promotion_action_failed');return result;};const render=()=>{if(type.dataset.active!=='discount-codes')return;list.replaceChildren();if(mode.dataset.active==='current'){const form=document.createElement('form');form.className='os-current-account-form';['Code Symbol','Discount Percentage','Number of Uses'].forEach(label=>{const input=document.createElement('input');input.value=label;input.readOnly=true;input.setAttribute('aria-label',label);form.append(input);});list.append(form);return;}const actions=document.createElement('div');actions.className='os-current-account-actions';actions.dataset.active=action;const indicator=document.createElement('span');indicator.className='os-current-account-actions-indicator';actions.append(indicator);[['create','addNew','ADD.svg'],['delete','deleteAccount','DELETE.svg'],['update','editExisting','EDIT.svg']].forEach(([name,label,icon])=>{const button=document.createElement('button');button.type='button';button.className='os-current-account-action';button.setAttribute('aria-label',t(label));const image=document.createElement('img');image.src='/assets/icons/New/'+icon;image.alt='';button.append(image);button.addEventListener('click',async()=>{action=name;selectedId='';deleteConfirm=false;if(name==='delete')try{await load();}catch(_){codes=[];}render();});actions.append(button);});list.append(actions);if(action==='create'||action==='update'){const form=document.createElement('form');form.className='os-current-account-form';['Code','Discount','Code Duration','Allowed Uses'].forEach(label=>{const input=document.createElement('input');input.placeholder=label;input.setAttribute('aria-label',label);form.append(input);});const controls=document.createElement('div');controls.className='os-current-account-confirm-actions';['cancel','confirm'].forEach(label=>{const button=document.createElement('button');button.type='button';button.className=label==='confirm'?'homepage-account-login':'os-current-account-cancel';button.textContent=t(label);button.addEventListener('click',()=>{if(label==='cancel'){action='';render();}});controls.append(button);});list.append(form,controls);}if(action==='delete'){const select=document.createElement('select');select.className='os-current-account-select';const initial=document.createElement('option');initial.value='';initial.textContent='Select Discount Code';select.append(initial);codes.forEach(code=>{const option=document.createElement('option');option.value=code.id;option.textContent=code.code_normalized;select.append(option);});select.value=selectedId;select.addEventListener('change',()=>{selectedId=select.value;deleteConfirm=false;render();});list.append(select);if(!selectedId)return;const controls=document.createElement('div');controls.className='os-current-account-confirm-actions';const cancel=document.createElement('button');cancel.type='button';cancel.className='os-current-account-cancel';cancel.textContent=t('cancel');cancel.addEventListener('click',()=>{selectedId='';deleteConfirm=false;render();});const removeButton=document.createElement('button');removeButton.type='button';removeButton.className='os-current-account-delete';removeButton.textContent=deleteConfirm?t('confirm'):'Delete code';removeButton.addEventListener('click',async()=>{if(!deleteConfirm){deleteConfirm=true;render();return;}try{await remove();await load();action='';selectedId='';deleteConfirm=false;render();}catch(_){deleteConfirm=false;render();}});controls.append(cancel,removeButton);list.append(controls);}};mode.querySelectorAll('button').forEach(button=>button.addEventListener('click',()=>{action='';selectedId='';deleteConfirm=false;setTimeout(render);}));type.querySelectorAll('button').forEach(button=>button.addEventListener('click',()=>{action='';selectedId='';deleteConfirm=false;setTimeout(render);}));render();})();</script>`;
