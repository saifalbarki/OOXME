...
const formatDate = value => value ? new Intl.DateTimeFormat('en', { year: 'numeric', month: 'short', day: 'numeric' }).format(new Date(value)) : '';
  ...
  const request = async body => {
    ...
    if (response.status === 401) { location.assign('/os'); throw new Error('unauthorized'); }
...
const dashboard = () => `${head('OOXME OS')}<body class="os-page"><main class="master-panel-experience os-experience os-dashboard-experience"><div class="master-panel-track os-panel-track">${dashboardPanel([['Website','website'],['Vercel','vercel'],['GitHub','github'],['Gmail','gmail'],['Drive','drive'],['Calendar','calendar'],['WhatsApp','whatsapp'],['Facebook','facebook'],['Instagram','instagram'],['YCloud','ycloud'],['GPT','gpt'],['Neon','neon']], true)}${currentAccountsPanel()}${panelThreeAccountsPanel()}${accountManagementPanel()}</div></main><script src="/js/os-language.js"></script><script src="/js/authenticated-navigation.js"></script>${dashboardScript}${currentAccountsScript}${panelThreeDiscountScriptLive}${panelThreeNotificationsScript}${panelStateResetScript}${accountManagementScript}</body></html>`;
