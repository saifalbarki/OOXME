...
const panelFooter = (leavesOs = false) => `<footer class="master-panel-footer homepage-bottom-navigation os-footer" data-authenticated-navigation><button class="master-panel-continue" type="button" data-auth-nav-trigger aria-label="Open navigation" aria-expanded="false"><span class="swipe-control"><span></span><span class="swipe-control-line"></span></span></button><nav class="homepage-bottom-menu" data-auth-nav-menu data-active="home" aria-label="Navigation"><span class="homepage-bottom-menu-indicator" aria-hidden="true"></span><button class="homepage-bottom-menu-button" type="button" data-auth-nav-item="account" data-auth-nav-destination="/os/accounts" aria-pressed="false"><img src="/assets/icons/New/PROFILE.svg" alt=""></button><button class="homepage-bottom-menu-button" type="button" data-auth-nav-item="gallery" aria-pressed="false"><img src="/assets/icons/New/GALLERY.svg" alt=""></button><button class="homepage-bottom-menu-button is-active" type="button" data-auth-nav-item="home" ${leavesOs ? 'data-auth-nav-exit' : 'data-auth-nav-destination="/os"'} aria-pressed="true"><img src="/assets/icons/New/HOME.svg" alt=""></button><button class="homepage-bottom-menu-button" type="button" data-auth-nav-item="search" aria-pressed="false"><img src="/assets/icons/New/SEARCH.svg" alt=""></button><button class="homepage-bottom-menu-button" type="button" data-auth-nav-item="menu" aria-pressed="false"><img src="/assets/icons/New/NOTIFICATION.svg" alt=""></button></nav></footer>`;
...
const dashboardSummary = () => `<section class="os-dashboard-summary" aria-label="Current totals"><div class="os-dashboard-summary-row"><span>Current Employees</span><strong class="os-dashboard-summary-active">4</strong><strong>6</strong></div><div class="os-dashboard-summary-row"><span>Clients</span><strong class="os-dashboard-summary-active">18</strong><strong>24</strong></div><div class="os-dashboard-summary-row"><span>Current Discount Codes</span><strong class="os-dashboard-summary-active">3</strong><strong>5</strong></div></section>`;
...
const panelThreeScript = `<script>(()=>{const mode=document.querySelector('[data-os-panel-three-mode]'),type=document.querySelector('[data-os-panel-three-type]'),list=document.querySelector('[data-os-panel-three-list]');if(!mode||!type||!list)return;const t=key=>window.OOXMEOS?.copy?.[window.OOXMEOS.language]?.[key]||key;let action='';const set=(selector,value,attribute)=>{selector.dataset.active=value;selector.querySelectorAll('button').forEach(button=>button.setAttribute('aria-selected',String(button.dataset[attribute]===value)));};const render=()=>{list.replaceChildren();if(mode.dataset.active==='current'){const empty=document.createElement('p');empty.className='os-current-accounts-empty';empty.textContent=t('noAccounts');list.append(empty);return;}const actions=document.createElement('div');actions.className='os-current-account-actions';actions.dataset.active=action;const indicator=document.createElement('span');indicator.className='os-current-account-actions-indicator';actions.append(indicator);[['create','addNew','ADD.svg'],['delete','deleteAccount','DELETE.svg'],['update','editExisting','EDIT.svg']].forEach(([name,label,icon])=>{const button=document.createElement('button');button.type='button';button.className='os-current-account-action';button.dataset.osAccountAction=name;button.setAttribute('aria-label',t(label));const image=document.createElement('img');image.src='/assets/icons/New/'+icon;image.alt='';button.append(image);button.addEventListener('click',()=>{action=name;render();});actions.append(button);});list.append(actions);if(action==='create'){const form=document.createElement('form');form.className='os-current-account-form';[['username','username'],['displayName','displayName'],['profile',type.dataset.active==='employee'?'jobTitle':'companyName'],['password','password']].forEach(([name,label])=>{const input=document.createElement('input');input.name=name;input.type=name==='password'?'password':'text';input.placeholder=t(label);form.append(input);});const controls=document.createElement('div');controls.className='os-current-account-confirm-actions';['cancel','confirm'].forEach(label=>{const button=document.createElement('button');button.type='button';button.className=label==='confirm'?'homepage-account-login':'os-current-account-cancel';button.textContent=t(label);button.addEventListener('click',()=>{if(label==='cancel'){action='';render();}});controls.append(button);});list.append(form,controls);}};mode.querySelectorAll('button').forEach(button=>button.addEventListener('click',()=>{set(mode,button.dataset.osPanelThreeModeOption,'osPanelThreeModeOption');action='';render();}));type.querySelectorAll('button').forEach(button=>button.addEventListener('click',()=>{set(type,button.dataset.osPanelThreeTypeOption,'osPanelThreeTypeOption');render();}));render();})();</script>`;
const panelThreeDiscountScript = `<script>(()=>{const mode=document.querySelector('[data-os-panel-three-mode]'),type=document.querySelector('[data-os-panel-three-type]'),list=document.querySelector('[data-os-panel-three-list]');if(!mode||!type||!list)return;const t=key=>window.OOXMEOS?.copy?.[window.OOXMEOS.language]?.[key]||key;let action='';const fields=['Code','Discount','Code Duration','Allowed Uses'];const render=()=>{if(type.dataset.active!=='discount-codes')return;list.replaceChildren();if(mode.dataset.active==='current'){const form=document.createElement('form');form.className='os-current-account-form';['Code Symbol','Discount Percentage','Number of Uses'].forEach(label=>{const input=document.createElement('input');input.value=label;input.readOnly=true;input.setAttribute('aria-label',label);form.append(input);});list.append(form);return;}const actions=document.createElement('div');actions.className='os-current-account-actions';actions.dataset.active=action;const indicator=document.createElement('span');indicator.className='os-current-account-actions-indicator';actions.append(indicator);[['create','addNew','ADD.svg'],['delete','deleteAccount','DELETE.svg'],['update','editExisting','EDIT.svg']].forEach(([name,label,icon])=>{const button=document.createElement('button');button.type='button';button.className='os-current-account-action';button.setAttribute('aria-label',t(label));const image=document.createElement('img');image.src='/assets/icons/New/'+icon;image.alt='';button.append(image);button.addEventListener('click',()=>{action=name;render();});actions.append(button);});list.append(actions);if(action==='create'||action==='update'){const form=document.createElement('form');form.className='os-current-account-form';fields.forEach(label=>{const input=document.createElement('input');input.placeholder=label;input.setAttribute('aria-label',label);form.append(input);});const controls=document.createElement('div');controls.className='os-current-account-confirm-actions';['cancel','confirm'].forEach(label=>{const button=document.createElement('button');button.type='button';button.className=label==='confirm'?'homepage-account-login':'os-current-account-cancel';button.textContent=t(label);button.addEventListener('click',()=>{if(label==='cancel'){action='';render();}});controls.append(button);});list.append(form,controls);}if(action==='delete'){const select=document.createElement('select');select.className='os-current-account-select';const option=document.createElement('option');option.textContent='Select Discount Code';select.append(option);list.append(select);}};mode.querySelectorAll('button').forEach(button=>button.addEventListener('click',()=>setTimeout(render));type.querySelectorAll('button').forEach(button=>button.addEventListener('click',()=>{action='';setTimeout(render);}));render();})();</script>`;
const panelThreeNotificationsScript = `<script>(()=>{const mode=document.querySelector('[data-os-panel-three-mode]'),type=document.querySelector('[data-os-panel-three-type]'),list=document.querySelector('[data-os-panel-three-list]');if(!mode||!type||!list)return;let action='';const notes=[['Project update','Your project milestone has been updated.','Aug 22, 2026'],['New message','A new message is available for your review.','Aug 21, 2026'],['Service notice','Your OOXME service notification is ready.','Aug 20, 2026']];const render=()=>{if(type.dataset.active!=='notifications')return;list.replaceChildren();if(mode.dataset.active==='current'){notes.forEach(([title,message,date])=>{const row=document.createElement('article');row.className='os-current-account-row';const details=document.createElement('div');details.className='os-current-account-details';const heading=document.createElement('strong');heading.textContent=title;const copy=document.createElement('span');copy.textContent=message;const time=document.createElement('small');time.textContent=date;details.append(heading,copy,time);row.append(details);list.append(row);});return;}const actions=document.createElement('div');actions.className='os-current-account-actions';actions.dataset.active=action;const indicator=document.createElement('span');indicator.className='os-current-account-actions-indicator';actions.append(indicator);[['create','ADD.svg'],['delete','DELETE.svg'],['update','EDIT.svg']].forEach(([name,icon])=>{const button=document.createElement('button');button.type='button';button.className='os-current-account-action';const image=document.createElement('img');image.src='/assets/icons/New/'+icon;image.alt='';button.append(image);button.addEventListener('click',()=>{action=name;render();});actions.append(button);});list.append(actions);if(action==='create'||action==='update'){const form=document.createElement('form');form.className='os-current-account-form';['Notification Title','Publish Date','Notification Text'].forEach(label=>{const input=document.createElement('input');input.placeholder=label;form.append(input);});const audience=document.createElement('select');audience.className='os-current-account-select';['Audience','Clients','Employees','Everyone'].forEach(value=>{const option=document.createElement('option');option.textContent=value;audience.append(option);});form.append(audience);const controls=document.createElement('div');controls.className='os-current-account-confirm-actions';['Cancel','Confirm'].forEach(label=>{const button=document.createElement('button');button.type='button';button.className=label==='Confirm'?'homepage-account-login':'os-current-account-cancel';button.textContent=label;button.addEventListener('click',()=>{if(label==='Cancel'){action='';render();}});controls.append(button);});list.append(form,controls);}if(action==='delete'){const select=document.createElement('select');select.className='os-current-account-select';const option=document.createElement('option');option.textContent='Select Notification';select.append(option);list.append(select);}};mode.querySelectorAll('button').forEach(button=>button.addEventListener('click',()=>setTimeout(render)));type.querySelectorAll('button').forEach(button=>button.addEventListener('click',()=>{action='';setTimeout(render);}));render();})();</script>`;
