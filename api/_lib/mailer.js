...
async function sendBookingConfirmationEmail(booking) {
  ...
  if (!mailer) return { skipped: true, reason: 'smtp_not_configured' };
  ...
  const summary = [
    ...
    ['Total Amount / المبلغ الإجمالي', amount(booking.quote?.finalAmount, booking.quote?.currency)]
  ];
  const text = [`OOXME Booking Confirmation / تأكيد حجز OOXME`, `Reference / المرجع: ${reference}`, '', ...summary.map(([label, value]) => `${label}: ${value}`)].join('\n');
